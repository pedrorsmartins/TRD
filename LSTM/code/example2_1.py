...
df = pd.read_csv('airRohr/data/pm25/dump_esp8266-244085.csv')

t = np.array(range(1,df['Time'].size+1))
y = df['torget_pm25_iot'].to_numpy()
...
#from keras.preprocessing.sequence import TimeseriesGenerator
y_1=df['torget_pm25_nilu'].to_numpy()
new_y=array=np.transpose(np.vstack((y,y_1)))
#Xtrain, Ytrain= generate_dataset.windowed_dataset(new_y, input_window = iw, output_window = ow, stride = s)

y = y.reshape(-1, 1)

Xtrain, Ytrain= generate_dataset.windowed_dataset(y, input_window = iw, output_window = ow, stride = s)
