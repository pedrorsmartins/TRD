...
if __name__ == '__main__':
    wx_path = 'AirNet/data/example4.csv' #location of dataset( Note that we have no right to directly publish the datasets in our paper.)
    wx_gk = pd.read_csv(wx_path, sep=',')
    wx_title = wx_path.split('/')[-1].split('.')[0]
    model_path = 'AirNet/data/model'
    check_or_create_path(model_path)
    # trond_path = 'AirNet/data/dump_with_all_features.csv'
