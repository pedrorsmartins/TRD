...
class EncoderRNN(nn.Module):
    def __init__(self, args, input_size):
        ...
        padding_conv2d=(math.floor(self.Ck/2),0)
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=self.hidC, kernel_size=(self.Ck, self.m), padding=padding_conv2d);
...
#            total_loss +=  evaluateL2(output , Y ).data[0]
            total_loss +=nn.functional.mse_loss(output, Y).item()
            total_loss_l1+=nn.functional.l1_loss(output, Y).item()
